...
export const analyzeResume = async (req, res) =>{
    ...
    res.json({
        ...
        return res.status(500).json({message:`analyzeResume error ${error.message}`})
    }
}
     ...
     try {
        ...
        const messages = [

            {
                ...
                content: `
                    ...
                    Speak in simple, natural English as if you are directly talking to the candidate, and occasionally use simple Hindi words or short Hindi phrases for a more natural conversation.

                    Generate exactly 15 interview questions.

                    Strict Rules:
                    - Each question must contain between 15 and 35 words.
        ...
        const interview  = await Interview.create({
             ...
             questions: questionsArray.map((q, index)=>({
                 question:q,
                 difficulty: ["esay","esay","esay","medium","medium","medium","hard"][index],
                 timeLimit:[60,60,60,90,90,90,120][index],
             }))
        })
...
export const getInterviewReport = async (req, res) => {
     try {
        ...
        return res.status(500).json({
            message:`failed to find currentUser interviews report ${error}`
        })
     }
}